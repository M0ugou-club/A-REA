...
export const actionsTriggersX = async (userId, type, data, accessToken, areaId) => {
...
const xActionsNewPostMe = async (data, accessToken, areaId) => {
    try {
        const response = await fetch(`https://api.twitter.com/2/users/by/username/me?user.fields=public_metrics`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${accessToken}`,
                'Content-Type': 'application/json'
