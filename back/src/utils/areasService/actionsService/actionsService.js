...
export const actionsChanges = async (platform, type, data, userId, areaId) => {
    ...
    switch (platform) {
        case 'Spotify':
            ...
            if (await actionsTriggersSpotify(userId, type, data, accessToken, areaId) === true) {
                return true;
            }
            return false;
        case 'OpenMeteo':
        ...
        case 'Youtube':
            ...
            if (await actionsTriggersYoutube(userId, type, data, accessToken, areaId) === true) {
                return true;
            }
            return false;
        case 'X':
            accessToken = await getAccesTokensServiceByUserId('X', userId);
            if (await actionsTriggersX(userId, type, data, accessToken, areaId) === true) {
                return true;
            }
        case 'Twitch':
            accessToken = await getAccesTokensServiceByUserId('Twitch', userId);
            if (await actionsTriggersTwitch(userId, type, data, accessToken, areaId) === true) {
                return true;
            }
            return false;
        default:
