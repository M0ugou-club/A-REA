...
export const actionsTriggersTwitch = async (
  userId,
  type,
  data,
  accessToken,
  ...
  try {
    switch (type) {
      case "on_live_twitch_inox":
        if (
          (await twitchActionsOnliveInox(data, accessToken, areaId)) === true
        ) {
          return true;
        }
        return false;
      case "on_live_twitch_michou":
        if (
          (await twitchActionsOnliveMichou(data, accessToken, areaId)) === true
        ) {
          return true;
        }
        return false;
    ...
    if (error.message.includes("401")) {
      ...
      return await actionsTriggersTwitch(
        userId,
        type,
        data,
        newAccessToken,
...
const twitchActionsOnliveInox = async (data, accessToken, areaId) => {
  const response = await fetch(
    "https://api.twitch.tv/helix/streams?user_login=Inoxtag",
    {
      method: "GET",
      headers: {
  ...
  if (response.ok) {
    ...
    if (lives.data[0]) {
      ...
      if ((live != null && live != data) || (live != null && data === null)) {
        await Actions.findOneAndUpdate({ _id: areaId }, { data: live });
        if (live) {
          return true;
        } else {
          return false;
...
const twitchActionsOnliveMichou = async (data, accessToken, areaId) => {
  const response = await fetch(
    "https://api.twitch.tv/helix/streams?user_login=Michou",
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${accessToken}`,
