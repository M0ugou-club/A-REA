...
export const actionsTriggersSpotify = async (userId, type, data, accessToken, areaId) => {
    try {
        switch (type) {
            ...
            case 'on_new_album_spotify':
                if (await spotifyActionsNewAlbum(data, accessToken, areaId) === true) {
                    return true;
                }
                return false
        ...
        if (error.message.includes('401')) {
            ...
            return await actionsTriggersSpotify(userId, type, data, newAccessToken, areaId);
        }
        console.log(error);
    }
    return false
