...
export const actionsTriggersYoutube = async (
  userId,
  type,
  data,
  accessToken,
  ...
  try {
    switch (type) {
      case "on_new_video":
        if (
          (await youtubeActionsNewVideo(data, accessToken, areaId)) === true
        ) {
          return true;
        }
        return false;
      ...
      case "on_ten_millions":
        if (
          (await youtubeActionsTenMillions(data, accessToken, areaId)) === true
        ) {
          return true;
        }
        return false;
    ...
    if (error.response && error.response.status === 401) {
      ...
      return await actionsTriggersYoutube(
        userId,
        type,
        data,
        newAccessToken,
    ...
    if (video) {
      ...
      if (title) {
        if (title != data || data === null) {
          await Actions.findOneAndUpdate({ _id: areaId }, { data: title });
          return true;
        }
      }
    }
