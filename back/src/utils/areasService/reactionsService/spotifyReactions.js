...
const likeMusic = async (accessToken) => {
  const response = await fetch(
    `https://api.spotify.com/v1/me/tracks?ids=1B0AVsL9wiQn8PfzLzKluH`,
    {
      method: "PUT",
      headers: {
        Authorization: `Bearer ${accessToken}`,
...
const addMusicToQueue = async (accessToken, deviceId) => {
  const response = await fetch(
    `https://api.spotify.com/v1/me/player/queue?uri=spotify:track:1B0AVsL9wiQn8PfzLzKluH&device_id=${deviceId}`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${accessToken}`,
